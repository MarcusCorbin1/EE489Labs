...
int main(void)
{
  ...
  /* add queues, ... */
	osMailQDef(myMailHandle, 3, xData);
	myMailHandle = osMailCreate(osMailQ(myMailHandle), NULL);
  /* USER CODE END RTOS_QUEUES */

  /* Create the thread(s) */
  /* definition and creation of Thread1 */
  osThreadDef(Thread1, SenderThread, osPriorityNormal, 0, 128);
  Thread1Handle = osThreadCreate(osThread(Thread1), (void *)&(xStructsToSend[0]));
  ...
  Thread2Handle = osThreadCreate(osThread(Thread2), (void*)&(xStructsToSend[ 1 ]));
...
void SenderThread(void const * argument)
{
  ...
  for(;;)
  {
    xStatus = osMailPut(myMailHandle, (xData *)argument);
		if(xStatus != osOK)
		{
			printf("\n\rCould not send to the Mail\n\r");
		}
...
void ReceiverThread(void const * argument)
{
  ...
  for(;;)
  {
   event = osMailGet(myMailHandle, 100);
		if(event.status == osEventMail)
		{
			xReceivedStruct = event.value.p;
			if(xReceivedStruct->ucSource == mainSENDER_1)
			{
				printf("\n\rFrom Sender1 = %ld\n\r", (long)(xReceivedStruct->ucValue));
			}
			else
			{
				printf("\n\rFrom Sender2 = %ld\n\r", (long)(xReceivedStruct->ucValue));
		...
		osMailFree(myMailHandle, xReceivedStruct);
